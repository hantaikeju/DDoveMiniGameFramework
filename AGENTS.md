...
知识在 `DDoveMiniGameWiki/`（OKF bundle）。**工程用法**在 `DDoveMiniGameWiki/01`–`04`；**需求稿**在 `DDoveMiniGameWiki/05-需求/`；**AI 用法**在 `DDoveMiniGameWiki/00-索引/Agent/`；约定在 `DDoveMiniGameWiki/00-索引/OKF约定.md`。规范原文是仓库根 `SPEC.md`（整文件替换后跟官方走）。
...
6. 需求未对齐：`/ddove-grill`；落稿 `/ddove-wiki-create`（`05-需求`，`draft` + `需求`，得分记在该篇）；已有篇过期 `/ddove-wiki-update`；整份需求做完 `/ddove-wiki-change-state`（生成到职种目录）。见 `DDoveMiniGameWiki/00-索引/Agent/需求稿工作流.md`。地图：`DDoveMiniGameWiki/00-索引/Agent/Agent总览.md`。
