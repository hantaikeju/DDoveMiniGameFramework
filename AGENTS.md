...
知识在 `wiki/`（OKF bundle）。**工程**在 `wiki/01`–`04`；**AI 用法**在 `wiki/00-索引/Agent/`；约定在 `wiki/00-索引/OKF约定.md`。规范原文是仓库根 `SPEC.md`（整文件替换后跟官方走）。
...
6. 需求未对齐：`/ddove-grill`；落稿 `/ddove-wiki-create`（`draft` + `需求`）；入正式库 `/ddove-wiki-change-state`。见 `wiki/00-索引/Agent/需求稿工作流.md`。地图：`wiki/00-索引/Agent/Agent总览.md`。
