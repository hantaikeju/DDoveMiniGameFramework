...
{
    public static class OpenLoopHDemoMenu
    {
        const string ScenePath = "Assets/GameResExcluded/CreateUIScenes/Start/WndLoopHDemo.unity";
        const string PrefabPath = "Assets/GameRes/UI/Start/WndLoopHDemo.prefab";
        const int DemoCount = 80;

        [MenuItem("Game/Open WndLoopHDemo")]
        public static void Open()
        {
            if (!Application.isPlaying)
            {
                Debug.LogWarning("[WndLoopHDemo] open in Play Mode.");
                return;
            }

            DDoveUIKit.OpenAsync<WndLoopHDemo>().Forget();
        }

        [MenuItem("Game/Build WndLoopHDemo Assets")]
        public static void BuildAssets()
        {
            if (string.IsNullOrEmpty(DDoveUISceneEditor.EnsureScene("WndLoopHDemo", "Start", DDoveUIPanelKind.Panel)))
            {
                Debug.LogError("[WndLoopHDemo] ensure scene failed.");
                return;
            }
            ...
            if (uiRoot == null)
            {
                Debug.LogError("[WndLoopHDemo] missing UIRoot.");
                return;
            }
            ...
            if (uiRoot.GetComponent<WndLoopHDemo>() == null)
            {
                uiRoot.AddComponent<WndLoopHDemo>();
            }

            if (!DDoveUIPrefabExporter.Save(uiRoot, "Start", "WndLoopHDemo"))
            {
                Debug.LogError("[WndLoopHDemo] export prefab failed: " + PrefabPath);
                return;
            }

            EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene(), ScenePath);
            AssetDatabase.Refresh();
            Debug.Log("[WndLoopHDemo] built " + ScenePath + " and " + PrefabPath + ", Create " + DemoCount);
        }
        ...
        static void BuildListTree(GameObject uiRoot)
        {
            ...
            templateRect.sizeDelta = new Vector2(100f, -16f);
            templateRect.anchoredPosition = new Vector2(8f, 0f);
            templateRect.localScale = Vector3.one;
            var button = template.GetComponent<Button>();
            button.navigation = new Navigation { mode = Navigation.Mode.None };
            template.AddComponent<DemoRow>();
            template.AddComponent<DefaultTween_H>();

            var anim = new GameObject("Anim", typeof(RectTransform), typeof(CanvasGroup), typeof(Image));
            ...
            var animImage = anim.GetComponent<Image>();
            ...
            so.FindProperty("direction").enumValueIndex = (int)LoopDirection.Horizontal;
