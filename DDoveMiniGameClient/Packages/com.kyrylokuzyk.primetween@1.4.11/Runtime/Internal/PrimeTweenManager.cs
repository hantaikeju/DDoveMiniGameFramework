...
namespace PrimeTween {
    ...
    internal sealed class PrimeTweenManager : MonoBehaviour {
        ...
        static void SaveMainThreadId() => _mainThreadId = Thread.CurrentThread.ManagedThreadId;
        [Conditional("UNITY_ASSERTIONS")]
        internal static void EnsureRunningOnMainThread() {
            if (Thread.CurrentThread.ManagedThreadId != _mainThreadId) {
                throw new Exception(Constants.nonMainThreadError);
            }
        }
        ...
        void Dispose() {
            if (!_isDestroyed) { // bypass main thread check
            ...
            void Add(TweenArray array) {
                foreach (var el in array) {
                    ...
                    list.Add(new TweenInspectorData {
                        ...
                        unityTarget = rt.target is UnityEngine.Object unObj && unObj != null ? unObj : cold.customOnValueChange as UnityEngine.Object,
                        isPaused = d.isPaused,
        ...
        static void IniOnLoad() {
            ...
            foundInScene._isDestroyed = false;
        ...
        internal void Update() {
            ...
            if (Time.timeScale <= 0f) {
                // FixedUpdate is not invoked while 'Time.timeScale == 0f', so we need to check cancellation tokens manually
                AddNewTweens(_UpdateType.FixedUpdate);
                foreach (var el in tweensFixedUpdate) {
                    ref var d = ref el.data;
                    if (d.isAlive && d.HasCancellationToken) {
                        var cold = el.tween.cold;
                        if (cold.cancellationToken.IsCancellationRequested) {
        ...
        static void AddNewTweens(TweenArray newTweens, TweenArray currentTweens) {
            ...
            if (newTweens.Count > 0) {
                foreach (var el in newTweens) {
                    ...
                    data.ModificationIsNotAllowed = true;
                    cold.data = data;
                }
                newTweens.Clear();
            }
                ...
                foreach (var el in tweens) {
                    ...
                    if (onTarget != null) {
                        ...
                        if (!allowToProcessTweensInsideSequence && d.isInSequence) {
                            if (onTarget is TweenAnimation) {
                                if (logCantManipulateError) {
                                    rt.LogErrorWithStackTrace($"Manipulating {nameof(TweenAnimation)} via this API is not supported, use {nameof(TweenAnimation)} directly.");
                                }
                            } else {
