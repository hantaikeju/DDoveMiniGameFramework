...
internal class TweenSettingsPropDrawer : PropertyDrawer {
    ...
    internal static void DrawEaseAndCycles(out int cycles, SerializedProperty property, ref Rect rect, bool addSpace = true, bool draw = true, GUIContent tweenAnimationCyclesGuiContent = null) {
        { // ease
            property.NextVisible(true);
            if (draw)
                EditorGUI.PropertyField(rect, property);
        ...
        { // cycles
            ...
            if (draw) {
                EditorGUI.PropertyField(rect, property, tweenAnimationCyclesGuiContent);
                bool allowInfiniteCycles = tweenAnimationCyclesGuiContent == null; // infinite cycles (-1) should not be allowed because TweenAnimation returns Sequence and Sequence can't have infinite nested animations
