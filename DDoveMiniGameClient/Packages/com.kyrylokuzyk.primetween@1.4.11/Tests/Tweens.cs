...
public partial class Tests {
    ...
    /// A 180° rotation serialized by the editor has w == 0 exactly, e.g. (0,1,0,0).
    /// This is a valid rotation and must not be confused with the (0,0,0,0) default/uninitialized quaternion.
    [UnityTest]
    public IEnumerator Quaternion180DegreesStartValue() {
        var target = new GameObject("rotation target").transform;
        var start = new Quaternion(0f, 1f, 0f, 0f); // exactly 180° about Y, w == 0
        Assert.AreEqual(0f, start.w);
        target.localRotation = start;
        var end = Quaternion.Euler(0f, 90f, 0f);
        Assert.Greater(Quaternion.Angle(start, end), 1f);
        var t = Tween.LocalRotation(target, end, 0.01f);
        yield return null; // startFromCurrent caches the diff on the first update
        // The captured start must remain the genuine 180° rotation, not be corrupted into the 90° end value.
        Assert.AreEqual(0f, Quaternion.Angle(start, t.tween.data.startValue.quaternion), 0.1f);
        t.Complete();
        Assert.IsTrue(Quaternion.Angle(target.localRotation, end) < 0.001f);
    ...
    [UnityTest]
    public IEnumerator WarnEndValueEqualsCurrentQuaternion() {
        Assert.AreEqual(0, tweensCount);
        var rot = Quaternion.Euler(0f, 90f, 0f);
        // -q represents the same rotation as q (quaternion double-cover), so the endValue equals the current value.
        var negated = new Quaternion(-rot.x, -rot.y, -rot.z, -rot.w);
        Assert.AreEqual(0f, Quaternion.Angle(rot, negated), 0.001f);

        transform.localRotation = rot;
        var t = Tween.LocalRotation(transform, negated, 1f);
        yield return null;
        LogAssert.Expect(LogType.Warning, new Regex("Tween's 'endValue' equals to the current animated value"));
        t.Stop();
    ...
    [UnityTest]
    public IEnumerator WarnEndValueEqualsCurrentQuaternionDifferentRotation() {
        Assert.AreEqual(0, tweensCount);
        var rot = Quaternion.Euler(0f, 90f, 0f);
        var differentRot = Quaternion.Euler(0f, 91f, 0f); // a genuinely different rotation must not trigger the warning
        Assert.Greater(Quaternion.Angle(rot, differentRot), 0.5f);

        transform.localRotation = rot;
        var t = Tween.LocalRotation(transform, differentRot, 1f);
        yield return null;
        t.Stop();
    ...
    [UnityTest]
    public IEnumerator NonMainThreadTween() {
        Thread thread = null;
        thread = new Thread(() => {
            Tween t = default;

            ExpectNonMainThreadException(() => Tween.Delay(0.01f));
            ExpectNonMainThreadException(() => Tween.Position(transform, default));
            ExpectNonMainThreadException(() => Tween.Custom(transform, new TweenSettings<Vector3>(), delegate {}));

            ExpectNonMainThreadException(() => { _ = t.isAlive; });

            ExpectNonMainThreadException(() => t.Stop());
            ExpectNonMainThreadException(() => t.Complete());

            ExpectNonMainThreadException(() => t.SetRemainingCycles(1));
            ExpectNonMainThreadException(() => t.SetRemainingCycles(true));

            ExpectNonMainThreadException(() => { _ = t.cyclesDone; });
            ExpectNonMainThreadException(() => { _ = t.cyclesTotal; });

            ExpectNonMainThreadException(() => { _ = t.isPaused; });
            ExpectNonMainThreadException(() => t.isPaused = true);

            ExpectNonMainThreadException(() => { _ = t.timeScale; });
            ExpectNonMainThreadException(() => t.timeScale = 1f);

            ExpectNonMainThreadException(() => { _ = t.duration; });
            ExpectNonMainThreadException(() => { _ = t.durationTotal; });

            ExpectNonMainThreadException(() => { _ = t.elapsedTime; });
            ExpectNonMainThreadException(() => t.elapsedTime = 1f);
            ExpectNonMainThreadException(() => { _ = t.elapsedTimeTotal; });
            ExpectNonMainThreadException(() => t.elapsedTimeTotal = 1f);

            ExpectNonMainThreadException(() => { _ = t.progress; });
            ExpectNonMainThreadException(() => t.progress = 0.5f);

            ExpectNonMainThreadException(() => t.OnComplete(transform, delegate {}));
            ExpectNonMainThreadException(() => t.OnComplete(delegate {}));

            ExpectNonMainThreadException(() => t.Group(new Tween()));
            ExpectNonMainThreadException(() => t.Chain(new Tween()));
            ExpectNonMainThreadException(() => t.Group(new Sequence()));
            ExpectNonMainThreadException(() => t.Chain(new Sequence()));

            ExpectNonMainThreadException(() => t.OnUpdate(transform, delegate {}));

            ExpectNonMainThreadException(() => t.SetCancellationToken(CancellationToken.None));

            ExpectNonMainThreadException(() => { _ = t.interpolationFactor; });

            ExpectNonMainThreadException(() => t.ResetOnCompletion());
    ...
    [UnityTest]
    public IEnumerator NonMainThreadSequence() {
        Thread thread = null;
        thread = new Thread(() => {
            ExpectNonMainThreadException(() => Sequence.Create());
            Sequence t = default;
            ...
            // t.OnUpdate() is not supported on Sequence

            ExpectNonMainThreadException(() => t.SetCancellationToken(CancellationToken.None));

            // t.interpolationFactor is not supported on Sequence

            #if PRIME_TWEEN_EXPERIMENTAL
            ExpectNonMainThreadException(() => t.ResetOnCompletion());

            ExpectNonMainThreadException(() => t.GroupCallback(delegate {}));
            ExpectNonMainThreadException(() => t.GroupCallback(transform, delegate {}));
            #endif


            ExpectNonMainThreadException(() => t.ChainCallback(delegate {}));
            ExpectNonMainThreadException(() => t.ChainCallback(transform, delegate {}));

            ExpectNonMainThreadException(() => t.Insert(0f, new Tween()));
            ExpectNonMainThreadException(() => t.Insert(0f, new Sequence()));

            ExpectNonMainThreadException(() => t.InsertCallback(0f, delegate {}));
            ExpectNonMainThreadException(() => t.InsertCallback(0f, transform, delegate {}));
    ...
    static void ExpectNonMainThreadException(TestDelegate testDelegate) {
        var ex = Assert.Throws<Exception>(testDelegate);
        Assert.AreEqual(Constants.nonMainThreadError, ex.Message);
    }
}
#endif
