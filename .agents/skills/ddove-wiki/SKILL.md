...
name: ddove-wiki
description: >-
  只检索本库 wiki/：grep frontmatter、读命中篇、列出要点。
...
约定：`wiki/00-索引/OKF约定.md`。本 skill 不写文件、不改代码、不给五段业务答案。干活走 `ddove-work`。
...
工程收窄 `wiki/01`–`04`；AI / skill 收窄 `wiki/00-索引/Agent`。只读 `OKF约定.md`，不走 `index.md` → `index_<短号>`。
