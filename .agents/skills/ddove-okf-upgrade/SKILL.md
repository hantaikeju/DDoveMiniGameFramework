...
| `wiki/**/log.md` | 最新日期在上；本库根 `log.md` 只作日期索引，明细在 `_log/log_YYYY-MM-DD_<短号>.md` |
...
6. 在 `wiki/_log/log_YYYY-MM-DD_<短号>.md` **顶部**追加当天条目（无则新建）；根 `wiki/log.md` 若还没有该日 `##` 节则加在最上面。写：SPEC x → y，改了哪些文件。
