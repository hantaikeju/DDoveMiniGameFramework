...
`[DDoveEditorPanel("architecture", "Architecture", 150)]`，挂在 [DDove Editor](/02-程序-前/DDoveEditor.md) 侧栏。侧栏顺序只改 `DDoveEditorNav.Ids`（当前 Architecture / HotBox / Res / UI）。实现放 `DDoveFramework.Editor`。**不要**再开独立 `EditorWindow`。**不要**引用 `DDoveUI`。
