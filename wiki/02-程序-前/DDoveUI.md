...
Play
    ...
    GameLaunch → GameArchitecture.Interface → DDoveUIKit.Initialize → OpenAsync<WndHome>
...
`GameLaunch`：碰 `Interface`（触发 Init）→ `DDoveUIKit.Initialize` → `OpenAsync<WndHome>`。正式游戏换成自己的入口，删或改这一份占位。
...
2. 总窗侧栏是 Architecture / HotBox / Res / UI（见 [DDove Editor](/02-程序-前/DDoveEditor.md) `DDoveEditorNav.Ids`）。UI 页改分辨率写进 `DDoveUIInitInfo`。饼环在 HotBox 页编。
