...
实现 `IDDoveEditorPanel`，类上挂 `[DDoveEditorPanel(id, title, order)]`。门面用 `TypeCache` 收集，**不要**改窗口源码、不要 `#if` 往里硬塞。侧栏顺序只改 [`DDoveEditorNav.Ids`](../../DDoveMiniGameClient/Assets/DDoveFramework/Editor/DDoveEditorNav.cs)（当前 Architecture / HotBox / Res / UI）；未进表的页仍按 `order` 排在后面。
