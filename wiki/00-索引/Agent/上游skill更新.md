...
| `grill-with-docs` / `domain-modeling` | `wiki/` + create | **不跟**落盘（禁止 `CONTEXT.md` / ADR） |
...
3. 值得跟：`@ddove-writing-for-agents` 改对应 `ddove-*`，同步 `.cursor/skills/`。过改写过滤器：`CONTEXT.md`→`wiki/`；issue tracker 删；`ddove-` 前缀保留。
